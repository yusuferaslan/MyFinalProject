...
namespace Core.Aspects.Autofac.Caching
{
    public class CacheAspect : MethodInterception
    {
        ...
        {
            var methodName = string.Format($"{invocation.Method.ReflectedType.FullName}.{invocation.Method.Name}"); //invocation.Method.ReflectedType.FullName >> namespace + classın ismini verir. Method.Name>< metodun ismini verir. key oluşturmaya çalışıyoruz. örn:(Northwind.Business.IProductService.GetAll)
            var arguments = invocation.Arguments.ToList(); //metodun parametresi varsa listeye çevir
            var key = $"{methodName}({string.Join(",", arguments.Select(x => x?.ToString() ?? "<Null>"))})"; //parametre değeri varsa GetAll() içine ekle >> Northwind.Business.IProductService.GetAll(x) .. ?? >> yoksa "<Null>" ekle demek.
